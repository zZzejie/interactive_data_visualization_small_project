...
class Companies(Resource):
    def get(self, company_name): # retrieve the company name from the url
        import pandas as pd
        from statsmodels.tsa.ar_model import AutoReg
        # search for the company by Company Name
        cursor = companies.find_one_or_404({"name": company_name})
...
# this is for retrieving a company by name
api.add_resource(Companies, '/companies/<string:company_name>')
